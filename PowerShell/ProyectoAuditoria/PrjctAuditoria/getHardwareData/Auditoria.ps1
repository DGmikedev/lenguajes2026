...
if($Eject -eq 1){
    ...
    $ObjectSend = [PSCustomObject]@{
        CPU    = if( "ERROR EN MODULO" -eq (Getcpu))      { 
                    ...
                    (Getcpu)      
                    $dateHrs = GettimeHMS 
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA CPU INSERTADO CON EXITO" 
            } 
        RAM    = if( "ERROR EN MODULO" -eq (Getram))      { 
                    ...
                    (Getram)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA MEMORIA RAM INSERTADO CON EXITO"
            } 
        DISC   = if( "ERROR EN MODULO" -eq (Getdisk))     { 
                    ...
                    (Getdisk)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA PARTICIONES INSERTADO CON EXITO"
            } 
        EQPMET = if( "ERROR EN MODULO" -eq (Geteqp))      { 
                    ...
                    (Geteqp)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA EQUIPO FISICO INSERTADO CON EXITO"
            } 
        NET    = if( "ERROR EN MODULO" -eq (Getnetadp))   { 
                    ...
                    (Getnetadp)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA CONFIGURACION RED-NET INSERTADO CON EXITO"
                }
        ...
        FDISK  = if( "ERROR EN MODULO" -eq (Getfdisk))    { 
                    ...
                    (Getfdisk)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA DISCO(S) FISICOS INSERTADO CON EXITO"    
            } 
        IPCNF  = if( "ERROR EN MODULO" -eq (Getipnet))    { 
                    ...
                    (Getipnet)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA CONFIGURACION RED INSERTADO CON EXITO"   
            } 
        UUID   = if( "ERROR EN MODULO" -eq (Getuuid))     { 
                    ...
                    (Getuuid)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA UUID INSERTADO CON EXITO"     
            } 
        TMZONE = if( "ERROR EN MODULO" -eq (Gettimezone)) { 
                    ...
                    (Gettimezone)
                    $dateHrs = GettimeHMS
                    Add-Content -Path $lgPath  -Value "$dateHrs | DATA TIMEZONE AJUSTADO INSERTADO CON EXITO" 
                } 
    ...
    if ((Export-Objecttojson -OutPath $PathReportWName -DataIn $ObjectSend) -eq 1){ 
        ...
        Add-Content -Path $lgPath -Value "$dateHrs | $msg"
